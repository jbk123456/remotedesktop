...
public class SSLSocketChannel extends SocketChannel {
	...
	private static final Logger LOGGER = Logger.getLogger(SSLSocketChannel.class.getName());
	 ...
	 * @param log             The logger for debug and error messages. A
	 *                        {@code null} logger will result in no log operations.
	 * @throws IOException
	 */
	 ...
	 */
	@Override
	synchronized public int read(ByteBuffer applicationBuffer) throws IOException, IllegalArgumentException, PlaintextConnectionException {
		LOGGER.fine(String.format("read: %d, %d", applicationBuffer.position(), applicationBuffer.limit()));
		int intialPosition = applicationBuffer.position();

		int readFromChannel = sslEngineBuffer.unwrap(applicationBuffer);
		LOGGER.fine(String.format("read: from channel: %d", readFromChannel));

		if (readFromChannel < 0) {
			LOGGER.fine("read: channel closed.");
			return readFromChannel;
		} else {
			int totalRead = applicationBuffer.position() - intialPosition;
			LOGGER.fine(String.format("read: total read: %d", totalRead));
			return totalRead;
		}
	}
	 ...
	 */
	@Override
	synchronized public int write(ByteBuffer applicationBuffer) throws IOException, IllegalArgumentException, PlaintextConnectionException {
		LOGGER.fine("write:");
		...
		if (writtenToChannel < 0) {
			LOGGER.fine("write: channel closed");
			return writtenToChannel;
		} else {
			int totalWritten = applicationBuffer.position() - intialPosition;
			LOGGER.fine(String.format("write: total written: %d amount available in network outbound: %d", totalWritten,
